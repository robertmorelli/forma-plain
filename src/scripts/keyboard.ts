KeyBinding: {
    ...
    document.addEventListener(`keydown`, event => {
        ...
        for (const ele of Array.from(document.querySelectorAll<HTMLLabelElement>(`label`))) {
            if (eles.has(`${ele.textContent}`)) throw new Error(`Dumb retard error: shit a brick`);
