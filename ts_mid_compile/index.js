...
Main: {
    const ui = {};
    FormBinding: {
        const _uiForm = document.forms.ui;
        _uiForm.addEventListener(`change`, () => window.location.hash = `#${btoa(JSON.stringify(Object.assign(ui, Object.fromEntries((new FormData(_uiForm)).entries()))))}`);
        Object.defineProperties(ui, Object.fromEntries(Array.from((new FormData(_uiForm)).keys())
            .map(key => [
            key, {
